...
export default function AdminChart() {
	const [chartData, setChartData] =
		React.useState<{ date: string; price: number }[]>(initialChartData);
	...
	const fetchData = async () => {
		...
		try {
			...
			console.log(error);
			toast.error('Có lỗi xảy ra khi lấy dữ liệu Chart!');
		} finally {
			setLoading(false);
		}
	};
			...
			<CardHeader className='flex flex-col items-stretch space-y-0 border-b p-0 sm:flex-row'>
				<div className='flex flex-1 flex-col justify-center gap-1 px-6 py-5 sm:py-6'>
					<CardTitle>
						...
						>
							{loading ? (
								<Loader2 className='inline-block mr-2 size-4' />
							) : (
					...
					<CardDescription>
						Hiển thị doanh số bán hàng theo{' '}
						<DropdownMenu>
				...
				<div className='flex'>
					<p className='relative z-30 flex flex-1 flex-col justify-center gap-1 border-t px-6 py-4 text-left even:border-l sm:border-l sm:border-t-0 sm:px-8 sm:py-6'>
						Doanh thu: {total.price.toLocaleString()} đ
					</p>
				</div>
			</CardHeader>
			<CardContent className='px-2 sm:p-6'>
				{loading && (
					<p className='text-center'>
						<Loader2 className='inline-block mr-2' /> Đang lấy dữ liệu ...
					</p>
				)}
				{!loading && chartData.length === 0 && (
					<p className='text-center'>
						Chưa có dữ liệu của {numberOfMonths} tháng đổ lại
					</p>
				)}
				{!loading && chartData.length > 0 && (
					<ChartContainer
